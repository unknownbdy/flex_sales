...
class PodcastVideoDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ->eloquent($query)
            ->editColumn('created_at', function (PodcastEpisode $podcastVideo) {
                return  UtilityFacades::dateFormat($podcastVideo->created_at);
            })
            ->addColumn('action', function (PodcastEpisode $podcastVideo) {
                return view('podcast-video.action', compact('podcastVideo'));
            });
    }

    public function query(PodcastEpisode $model)
    {
    ...
    public function html()
    {
        return $this->builder()
            ...
            ->language([
                'buttons' => [
                    'create' => __('Create'),
                    // 'create' => '<a href="' . route('podcast-video.create', ['id' => $podcast->id]) . '" class="btn btn-primary">Create</a>',
                    'export' => __('Export'),
                    'print' => __('Print'),
                    'reset' => __('Reset'),
                    'reload' => __('Reload'),
    ...
    {
        return 'Podcast_Videos_' . date('YmdHis');
    }
}
