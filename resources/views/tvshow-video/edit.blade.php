            ...
            <div class="col-md-12 m-auto">
                <div class="card  rounded-card-10px">
                    <div class="card-header">
                        <h5> {{ __('Edit Tv-Show') }}</h5>
                        {{-- {{($user->title)}} --}}
                    </div>
                    {{ Form::model($user, ['route' => ['tvshow-video.update', $user->id], 'method' => 'PUT', 'enctype' => 'multipart/form-data','id'=>'editTVShowForm']) }}
                    <div class="card-body">
                    <div class="row">
                        <div class="form-group col-6">
                            {{ Form::label('topic_english', __('Topic English'),['class' => 'col-form-label']) }}
                            {!! Form::text('topic_english', null, ['placeholder' => __('Topic English'), 'class' => 'form-control']) !!}
                        </div>
                        <div class="form-group col-6">
                            {{ Form::label('topic_arabic', __('Topic Arabic'),['class' => 'col-form-label']) }}
                            {!! Form::text('topic_arabic', null, ['placeholder' => __('Topic Arabic'), 'class' => 'form-control']) !!}
                        </div>
                    </div>
                        <div class="row">
                            <div class="form-group col-6">
                    ...
                    <div class="card-footer" style="padding: 10px 0px !important">
                        <div class="d-flex align-items-center justify-content-end px-4 py-1 gap-2">
                            <a href="{{ route('tvshow-video.index') }}" class="btn btn-secondary mb-0">{{ __('Cancel') }}</a>
...
@push('scripts')
  ...
  function validateform() {
    ...
    const formElement = document.getElementById('editTVShowForm');
...
<script>
    function previewImage() {
        ...
        if (thumbnailInput.files && thumbnailInput.files[0]) {
            ...
            // Update hidden input value with new image
            var hiddenImageInput = document.getElementById('hiddenImageInput');
            hiddenImageInput.value = thumbnailInput.files[0].name;
        }
    }
</script>
